...
class Olmoe2Attention(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        assert isinstance(self.config, Olmoe2Config)
...
class Olmoe2DecoderLayer(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        layer_idx = extract_layer_index(prefix)
        if layer_idx in hf_config.mlp_only_layers:
            self.mlp = Olmoe2MLP(vllm_config=vllm_config,
            ...
            self.mlp = Olmoe2MLP(
                ...
                intermediate_size=hf_config.shared_mlp_intermediate_size,
                prefix=f"{prefix}.mlp")
            self.block_sparse_moe = Olmoe2MoE(
                vllm_config=vllm_config, prefix=f"{prefix}.block_sparse_moe")
            self.post_moe_norm = RMSNorm(hf_config.hidden_size,
                                         eps=hf_config.rms_norm_eps)
    ...
    def forward(
        ...
        if moe_hidden_states is not None:
        ...
        for name, loaded_weight in weights:
            ...
            for (param_name, weight_name, shard_id) in stacked_params_mapping:
                ...
                if weight_name not in name:
                    continue
                # We have block_sparse_moe.experts[0].gate_proj in the
                ...
                # block_sparse_moe.experts[0].gate_up_proj, which will then be
                # updated below in expert_params_mapping for
                # block_sparse_moe.experts[0].gate_gate_up_proj, which breaks
                # load.
